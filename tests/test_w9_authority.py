...
from sos import (
    AuthorityAction, AuthorityGrant, DecisionRequest, EvidenceQuality,
    ModelValidationError, Traceability, evaluate,
)
...
def grant(**overrides):
    values = dict(
        ...
        min_confidence=.8, max_risk=.2, max_blast_radius=.2, require_reversible=True,
...
def test_risk_or_blast_radius_exceeding_grant_becomes_ask():
    decision = evaluate(request(risk=.9), (grant(),))
    assert decision.outcome == AuthorityAction.ASK
