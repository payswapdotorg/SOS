...
from sos import (
    AssuranceResult, AssuranceVerdict, Experiment, ExperimentDesign, ExperimentStage,
    GuardrailTrigger, ImpactAssessment, ModelValidationError, RiskAssessment,
    Traceability, advance, promotion_from_assurance, rollback,
)
...
def assured_experiment():
    return advance(
        advance(experiment(), target=ExperimentStage.ANALYZED, transition_revision="2"),
        target=ExperimentStage.ASSURED, transition_revision="3", assurance=assurance(), assurance_ref="assurance:1",
    )
...
def test_lifecycle_requires_sequential_steps_and_assurance():
    ...
    with pytest.raises(ModelValidationError):
        advance(analyzed, target=ExperimentStage.ASSURED, transition_revision="rev-3", assurance=assurance(AssuranceVerdict.BLOCK), assurance_ref="assurance:1")
    with pytest.raises(ModelValidationError):
        advance(analyzed, target=ExperimentStage.ASSURED, transition_revision="rev-3", assurance=assurance())
    assured = assured_experiment()
    ...
    assert shadow.assurance_ref == "assurance:1"


def test_promotion_requires_explicit_authority_and_matching_assurance():
    exp = assured_experiment()
    for target, rev in ((ExperimentStage.TESTED, "4"), (ExperimentStage.REPLAYED, "5"), (ExperimentStage.SHADOW, "6"), (ExperimentStage.CANARY, "7"), (ExperimentStage.EXPERIMENTAL, "8")):
        exp = advance(exp, target=target, transition_revision=rev)
    with pytest.raises(ModelValidationError):
        advance(exp, target=ExperimentStage.PROMOTED, transition_revision="9", promotion=promotion_from_assurance(assurance(), assurance_ref="wrong", authority_ref="owner", authority_evidence_ref="auth", decision_revision="9"))
    promotion = promotion_from_assurance(assurance(), assurance_ref="assurance:1", authority_ref="owner-1", authority_evidence_ref="auth-e1", decision_revision="9")
...
def test_guardrail_can_record_rollback_from_live_stage():
    exp = assured_experiment()
    for target, rev in ((ExperimentStage.TESTED, "4"), (ExperimentStage.SIMULATED, "5"), (ExperimentStage.SHADOW, "6")):
        exp = advance(exp, target=target, transition_revision=rev)
