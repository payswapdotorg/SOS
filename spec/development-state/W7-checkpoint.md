...
| R9, R21 | C2 evidence-backed gate evaluation | `AssuranceGate` + `_evidence_to_gate_status` + `_aggregate_gate_status` (no PASS from non-PASS) | `test_gate_records_evidence_references_and_truthful_status`, `test_unknown_evidence_cannot_become_pass`, `test_unavailable_evidence_cannot_become_pass` |
...
| R10, R21 | C6 causal qualification | causal gate PASS only with intervention-grade W4 evidence | `test_observational_evidence_cannot_become_intervention_proof`, `test_intervention_evidence_can_pass_causal_gate` |
| R14 | C7 reversibility / containment | `ReversibilityAssessment` (recorded, not executed) | `test_reversibility_records_whether_rollback_available`, `test_reversibility_does_not_execute_rollback` |
...
Review the exact PR head and CI result against the W7 Work Order. On approval,
merge the reviewed head and reconcile canonical state to W8 eligibility (W8
depends on W7). Worker state: `WAITING_FOR_ARCHITECT`. No merge, no
