...
- exact repository-revision + source-path provenance on every recovered fact;
...
| R7, R23 | C2 typed W2 graph facts with provenance | `_build_graph` + `RecoveredFile` provenance | `test_recovery_produces_valid_w2_system_state_and_graph`, `test_recovered_facts_carry_source_path_and_revision_provenance`, `test_recovered_graph_uses_frozen_node_and_edge_vocab` |
...
| R24 | C6 determinism | content-addressed ids + sorted walk | `test_recovery_is_deterministic_same_bytes_same_revision`, `test_different_revision_produces_different_artifact_identity` |
...
Exact-head results (to be recorded in the PR description at push time):
...
The full suite is green: 8 W1 + 8 W2 + 18 W3 = 34 tests, 0 failures.
...
Review the exact PR head and CI result against the W3 Work Order. On approval,
merge the reviewed head and reconcile canonical state to W4 eligibility (W4 is
already independently eligible by roadmap dependency on W2). Worker state:
